...
function mergeObjects(objects) {
  ...
  let resObj = {};
  resObj = objects.reduce((acc, item) => {
 ...
 */
function makeWord(letterObject) {
  const arrStack = [];
  Object.entries(letterObject).forEach(([key, value]) => {
    value.forEach((item) => {
      arrStack[item] = key;
    });
  });
...
function sortCitiesArray(arr) {
  return arr.sort(function sorting(a, b) {
    const compareCountryResult = a.country.localeCompare(b.country);
    return compareCountryResult === 0
      ? a.city.localeCompare(b.city)
      : compareCountryResult;
  });
}
...
function group(array, keySelector, valueSelector) {
  const newMap = array.reduce((acc, item) => {
